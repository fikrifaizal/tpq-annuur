...
$query = "SELECT `id`,`nama` FROM `piket` WHERE `status` LIKE 'AKTIF' AND `tgl_daftar` BETWEEN '2022-01-01' AND '$setFilter'";
...
$pdf->setSubject('Absensi Petugas Piket');

// header & footer data
$pdf->setHeaderData('logo-annur-bulat-tanpa-alpha.jpg', 16, 'Informasi Absensi Petugas Piket', "Periode ".ucfirst(strtolower($dateData['bulan']))." ".$dateData['tahun'], array(8,138,68), array(0,0,0));
...
while($data = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
  ...
  for($i=1;$i<=$manyDays;$i++) {
    ...
    $cekPresensi = "SELECT keterangan FROM `presensi_piket`
                    WHERE tanggal LIKE '$today' AND piket_id LIKE '".$data['id']."'";
