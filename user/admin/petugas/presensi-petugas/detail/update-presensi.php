...
if(isset($_GET['id']) && isset($_GET['induk'])) {
  ...
  if($action == "insert") {
    if($keterangan == 1) {
      $queryRalat = "INSERT INTO `presensi_piket`(`piket_id`,`filter_id`,`keterangan`,`tanggal`) VALUES ('$induk','$id','HADIR','$tanggal')";
      $resultRalat = mysqli_query($conn, $queryRalat);
    
      header("Location: detail-presensi.php?id=$id&tgl=$filterDate");
    }
    elseif($keterangan == 0) {
      $queryRalat = "INSERT INTO `presensi_piket`(`piket_id`,`filter_id`,`keterangan`,`tanggal`) VALUES ('$induk','$id','TIDAK HADIR','$tanggal')";
      $resultRalat = mysqli_query($conn, $queryRalat);
