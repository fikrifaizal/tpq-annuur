 ...
 <body >
   ...
   <div style='height: 80%; overflow: auto;' >
     ...
     <table border='1' width='100%' style='border-collapse: collapse;margin-top: 20px;'>
       ...
       <?php
       $emp_query = "SELECT * FROM keuangan";
       ...
       if(isset($_POST['but_search'])){
          ...
          if(!empty($fromDate) && !empty($endDate)){
             $emp_query .= " WHERE tanggal between '".$fromDate."' and '".$endDate."' ";
          }
        }
        ...
        if(mysqli_num_rows($employeesRecords) > 0){
          while($empRecord = mysqli_fetch_assoc($employeesRecords)){
            $id = $empRecord['id'];
            $empName = $empRecord['keterangan'];
            $date_of_join = $empRecord['keluar'];
            $gender = $empRecord['masuk'];
            $email = $empRecord['tanggal'];
